...
@require_admin
def export_licenses():
    """Export licenses as TEXT file - Keys only for unused, full details for used/active"""
    try:
        ...
        # Separate licenses by status
        unused_licenses = [lic for lic in licenses if lic['status'] == 'unused']
        used_licenses = [lic for lic in licenses if lic['status'] in ('active', 'expired')]
        
        # Write unused licenses (keys only)
        if unused_licenses:
            output.write("="*60 + "\n")
            output.write("UNUSED LICENSE KEYS\n")
            output.write("="*60 + "\n\n")
            for lic in unused_licenses:
                output.write(f"{lic['license_key']}\n")
            output.write("\n")
        
        # Write used/active licenses (full details)
        if used_licenses:
            output.write("="*60 + "\n")
            output.write("USED/ACTIVE LICENSE KEYS (WITH DETAILS)\n")
            output.write("="*60 + "\n\n")
            for lic in used_licenses:
                output.write(f"License Key: {lic['license_key']}\n")
                output.write(f"Status: {lic['status']}\n")
                output.write(f"Type: {lic['license_type'] or 'discord_vm'}\n")
                output.write(f"Days: {lic['days']}\n")
                output.write(f"HWID: {lic['hwid'] or 'Not set'}\n")
                output.write(f"Created: {lic['created_at'] or 'N/A'}\n")
                output.write(f"Activated: {lic['activated_at'] or 'N/A'}\n")
                output.write(f"Expires: {lic['expires_at'] or 'N/A'}\n")
                output.write(f"Customer Email: {lic['customer_email'] or 'N/A'}\n")
                output.write(f"Customer Note: {lic['customer_note'] or 'N/A'}\n")
                output.write(f"Admin Notes: {lic['admin_notes'] or 'N/A'}\n")
                output.write("-"*60 + "\n\n")
        ...
        response.headers['Content-Disposition'] = f'attachment; filename=licenses_{status_filter}_{license_type}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
